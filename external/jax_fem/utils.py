...
def save_sol(fe, sol, sol_file, cell_infos=None, point_infos=None):
    ...
    if cell_infos is not None:
        for cell_info in cell_infos:
            ...
            # 支持标量和多维 cell data
            if data.shape == (fe.num_cells,):
                # 标量数据
                out_mesh.cell_data[name] = [data]
            elif len(data.shape) > 1 and data.shape[0] == fe.num_cells:
                # 多维数据（向量、张量等）
                # meshio 需要将数据包装在列表中，每个元素对应一个 cell type
                out_mesh.cell_data[name] = [data]
            else:
                raise ValueError(f"cell data wrong shape, get {data.shape}, expected first dimension to be {fe.num_cells}")
