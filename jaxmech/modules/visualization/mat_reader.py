...
def load_viz_data(
    ...
    *,
    ...
    vd.points = pts
    vd.cells = cells
    vd.cell_types = np.asarray(ctypes, dtype=np.int32).reshape(-1) if ctypes is not None else None
    vd.cell_ele_types = np.asarray(eletypes, dtype=object).reshape(-1) if eletypes is not None else None
    ...
    if vd.cell_ele_types is not None:
        try:
            if np.all(np.asarray(vd.cell_ele_types, dtype=object) == "GAUSS_POINT"):
                vd.n_gauss_per_elem = np.ones(int(np.asarray(vd.cell_types).reshape(-1).shape[0]), dtype=np.int32)
        except Exception:
            pass
